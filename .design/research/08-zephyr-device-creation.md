...
- [`/.design/research/09-ticktimer-config-adjudication.md`](/.design/research/09-ticktimer-config-adjudication.md) is the stable decision defining the fixed `clock-frequency` compile-time interface and configurable-rate envelope; per its 2026-08-19 addendum the accepted takeover proof is the cadence measurement (the earlier two-reset observe-zero proof is falsified), with derived bounded polling. This design preserves that interface; unrelated clock-provider work does not migrate ticktimer.
